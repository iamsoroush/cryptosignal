...
class CryptoTickFetcher:
    ...
    def fetch_from_id_to_time(self,
        ...
        :returns a list of pandas DataFrames, each one for a tick-per-candle, and contains each row as one candle with
            these columns: ['Open', 'High', 'Low', 'Close', 'Volume', 'DateTime'] and the last trade_id
        """
    ...
    def fetch(self,
        ...
        :returns a list of pandas DataFrames, each one for a tick-per-candle, and contains each row as one candle with
            these columns: ['Open', 'High', 'Low', 'Close', 'Volume', 'DateTime']
    ...
    def get_starting_id(self, base_currency, target_currency, time_in_ms):

        """Returns the starting trade_id given the starting time.

        :arg base_currency: e.g. ETH
        :arg target_currency: e.g. USDT
        :arg time_in_ms: start time-stamp in miliseconds.
    ...
    def _get_trades(self, symbol, last_id, max_retries=3):
    ...
    def get_df_for_trades(self, trades):
        ...
        if len(trades) >= trade_per_candle:
            for s in range(0, len(trades), trade_per_candle):
                ...
                if e > len(trades):
                    ...
                    date_time = miliseconds_timestamp_to_str(slce[-1]['T'])
